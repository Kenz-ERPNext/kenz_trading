...
frappe.ui.form.on("Sales Order", {
    ...
    refresh(frm) {
        ...
        // Item search shows all items via frm.set_query in first refresh handler
        ...
        frappe.call({
            ...
            // Main on/off switch
            if (settings.show_sale_search_window) {
        ...
        // frappe.db.get_single_value('Kenza Settings', 'show_sale_search_window').then(value => {
        ...
        //             if (value && cur_frm && cur_frm.doctype === "Sales Order") {
        //                 show_last_sales_dialog(cur_frm);
        //             }
        //         }
        //     });
        ...
        frappe.call({
            ...
            if (!is_purchase_restricted) {
                ...
                // Main switch
                if (settings.show_purchase_search_window) {
        ...
        // frappe.db.get_single_value('Kenza Settings', 'show_purchase_search_window').then(value => {
        ...
        frappe.call({
            ...
            if (!is_item_qty_restricted) {
                ...
                if (settings.show_item_qty_search_window) {
        ...
        // frappe.db.get_single_value('Kenza Settings', 'show_item_qty_search_window').then(value => {
...
function show_last_sales_dialog(frm) {
    ...
    d.fields_dict.get_data.onclick = () => {
        ...
        frappe.call({
            method: "kenz_trading.events.sales_order.get_last_sales_details",
            args: values,
            callback: function (r) {
                if (r.message) {
...
function show_last_purchase_dialog(frm) {
    ...
    d.fields_dict.get_data.onclick = () => {
        ...
        frappe.call({
            method: "kenz_trading.events.sales_order.get_last_purchase_details",
            args: values,
            callback: function (r) {
                if (r.message) {
...
function load_item_transaction_details(item_code, frm) {
    ...
    // Check Kenza Settings
    frappe.db.get_single_value(
        "Kenza Settings",
        "show_last_5_sales_and_purchase_transactions"
    ).then(enabled => {

        // Always call backend (we need sales in both cases)
        frappe.call({
            method: 'kenz_trading.events.sales_order.get_last_purchase_and_sales',
            ...
            callback: function (r) {
                ...
                if (enabled) {
                    ...
                    // Show ONLY Sales (Full Width)
                    html += `
                        <div class="transaction-box full-width">
                            <h4>Last 5 Sales</h4>
                            ${get_table_html(sales, "sales")}
                        </div>
                    `;
...
async function get_last_sales_rate(item_code) {
    ...
    const r = await frappe.call({
        method: "kenz_trading.events.sales_order.get_last_sales_invoice_rate",
...
// Item query for Sales Order items is set via frm.set_query in first refresh handler
